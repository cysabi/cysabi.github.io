...
## design # a scoring system that encourages going for kills

One of the biggest problems with limited lives in a free-for-all system is that it **prioritizes survival over getting kills**.

> I wanted a system that rewarded players for going in for kills, without having too much risk if they happen to not succeed.
...
So I decided on a scoring system that is **`+2` for kills**, and **`-1` for deaths**.

## design # player accessibility with item rubber-banding
...
**`Here's How:`** Inspired by Mario Kart, item rubber-banding keeps the game an enjoyable experience for players far behind. **There's always a sense of agency from utilizing your items strategically to make an impact on the game**.

## design # preserving counterplay for all players
...
**`Here's How:`** Players can hold up to 8 items at a time. If a player is already holding 8, they won't be able to recieve new items.

## design # dropping items without causing a wealth gap

I wanted it to be important that players be able to target other players and if they succeed, to remove their items.
...
**`Here's How:`** I chose to make **the percentage of items you lose decrease the futher behind you are**. So a player in the lead would lose 100% of their items, while a player in last place only loses 12.5%.

## design # encouraging emergent vertical play
...
**`Here's How:`** I was able to fulfill that 3d combat space fantasy by **introducing a plethora of different quick-cast mobility items**. There are consumable double-jumps, levitation effects, and even one that automatically builds a bridge.

## design # using items to influence the game's atmosphere

While the main role of items is to provide accesibility, they have a lot of utility as a tool that I can use to **add a healthy amount of chaotic, party-like energy to the game's atmosphere** in a way that feels both **strategic and chaotic in equal proportions**.

![An early draft of item spawn rates depending on points behind](/block-sumo_2.webp)

**`Here's How:`** By being able to fine-tune the exact frequency of items at every level, I have a lot of control over the atmosphere and the level of rubber-banding.

# iteration

## design # rebalancing snowballs and player threat-ranges

Because of the abundance of snowballs that are given to first place _(as the "coin" / "green shell" of this game)_ it became an effective strategy for whoever was in 1st place to retreat constantly while kiting opponents away with an infinite supply of snowballs.

This was exacerbated because most of the high-power items required close range combat for them to get value. This was an intentional decision, because **I didn't want those behind to find an easy strategy of staying on the outskirts with high-power ranged weapons to catch up**.

- I couldn't simply lower the quantity of snowballs, as that would also affect those that weren't in first place.
...
2. ### players in the lead mostly recieve 4-count snowballs, instead of the previous 8.
   ...
   This change removed the spamming power for first place, so **players have to be more intentional about when they decide to use them**, but it keeps it's purpose as a "useful when used right" defensive tool.
...
5. ### introduce a new item, a powerful, but rare, long-ranged projectile (that's more effective at further ranges).
...
It was important to first decide what goals the map should have. I wanted the map to provide opportunities for further skill expression through the deliberate placement of platform skips and wall-jump pillars (both of which use blocks).
...
As of right now, this game is not publically avaliable to play, it's currently just a passion project to help me **learn game design and development for competitive multiplayer games**. I also just wanted to make a game that I could play with my friends at any time. Being able to do playtesting with my friends whenever we were bored always provided me with invaluable feedback and I'm really grateful for that.

However, I do have something to share! In order to grab footage for this portfolio, I hosted another playtest with a different group of friends that have never played the game before, and here's what they had to say!
