export const data = {
  ...
  tags: ["library", "software design"],
  previews: ["/blurple-py_1.png"],
}

![The documentation page for blurple.py](/blurple-py_1.png)

While making my Off the Dial bot, I realized that a lot of the code I made was actually very useful and could be extracted, refactored, and published as a library, so that's exactly what I did.

## designing an extensible system for developers of multi-step commands

![A couple examples of the reply system in action](/blurple-py_5.webm)
...
In fact, the total line count for [toast.py](https://github.com/cerusabi/blurple.py/blob/main/blurple/ui/toast.py) is just **_60 lines of code!_**, including all documentation and whitespace! That's the difference designing a well thought out API makes.
...
## designing a clean user interface for components

When it comes to designing visual elements of discord bots, you don't really have a lot of flexibility, as you only have whatever options the embed has. But that doesn't mean you can't make embeds look fancy!

Most of the the design components for discord bots usually fall into the category of "feedback". For example, stuff like `Alert`s and `Toast`s!

I took inspiration from the semantic styling of alerts in Bootstrap, and that was what I based my `ui.Style` options from.

![Different style options for alerts!](/blurple-py_3.png)
...
I'm really happy with how clean the user-facing API is, and how concise the router code is (less than [60 lines](https://github.com/cerusabi/blurple.py/blob/main/blurple/ext/router.py))!
