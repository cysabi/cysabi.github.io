export const data = {
  name: "cg-fabl",
  subtitle: "stream overlays for a unique solo-registration major tournament",
  ...
  desc: (
    <>
      ...
      <p>
        features include pulling live data from an api for it's leaderboard
        scene, and fluid and dynamic full layout-wide animations.
      </p>
    </>
  ),
  roles: ["design", "code"],
  ...
  sources: {
    repo: "https://github.com/cq-overlays/cg-fabl",
  },
}
...
This step was the one that took much longer than expected, it took a lot of time to **define the hierarchy of information**.
...
At this point, I had about 6 hours left before the tournament started, so I had to pick what animations were important enough to show, the ones that were most common to be noticed by the viewer. Everything else would be delegated to next week (this event ran over the course of 6 weeks).
...
Learning how to crunch and work with another designer to build broadcast graphics at a very fast pace was a valuable experience! But if I were to do it again, I would have given myself a bit more time to build out the graphics.
