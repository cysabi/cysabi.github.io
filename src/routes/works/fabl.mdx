export const data = {
  ...
  desc: (
    <>
      <p>
        i was tasked with building a website, and a set of broadcast graphics
        for a splatoon major tournament. it includes a unique "showdown" scene
        for finals day, with custom map states such as pick/ban, and a
        leaderboard scene that connects to my website's data api.
      </p>
    </>
  ),
  roles: ["design", "code"],
